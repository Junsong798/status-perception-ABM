# Simulation 0 with partial random cost
# built upon "simulation0_failure_cost_yield.R"
# replace the random_cost parameter with a random_prop
...
coalitional_aggres_single_gen <- function(
  ...
  # validate random_cost_prop
  if (!is.numeric(random_cost_prop) || length(random_cost_prop) != 1 ||
      random_cost_prop < 0 || random_cost_prop > 1) {
    stop("random_cost_prop must be a numeric value between 0 and 1.")
  }
  ...
  for (r in seq_len(round_bouts)) {
    ...
    # compute expected values with mixing between random and deterministic cost allocation
    # assume that the proportion applies to both cost_fight and cost_lose
    
    ## i: EV calculations
    ev_i_solo <- reward * p_i_solo - cost_fight - cost_lose * (1 - p_i_solo)
    
    if (types[i] == "C" && !is.na(i_ally)) {
      # total strength of hypothesized coalition
      coal_i_stren_total <- stren[i] + stren[i_ally]
      # deterministic share for focal: (1 - focal_strength / total)
      det_share_factor_i <- 1 - stren[i] / coal_i_stren_total
      
      # Expected random share for focal (random allocation between two members) = cost / 2
      exp_rand_fight_share <- cost_fight / 2
      exp_rand_lose_share <- cost_lose / 2
      
      # blended expected costs
      exp_fight_cost_i <- random_cost_prop * exp_rand_fight_share +
        (1 - random_cost_prop) * (cost_fight * det_share_factor_i)
      
      exp_lose_cost_i <- random_cost_prop * exp_rand_lose_share +
        (1 - random_cost_prop) * (cost_lose * det_share_factor_i)
      
      ev_i_coal <- reward * p_i_coal / 2 -
        exp_fight_cost_i -
        exp_lose_cost_i * (1 - p_i_coal)
    } else {
      ev_i_coal <- -Inf
    }
    ...
    if (types[j] == "C" && !is.na(j_ally)) {
      ...
      det_share_factor_j <- 1 - stren[j] / coal_j_stren_total
      
      exp_rand_fight_share_j <- cost_fight / 2
      exp_rand_lose_share_j <- cost_lose / 2
      
      exp_fight_cost_j <- random_cost_prop * exp_rand_fight_share_j +
        (1 - random_cost_prop) * (cost_fight * det_share_factor_j)
      
      exp_lose_cost_j <- random_cost_prop * exp_rand_lose_share_j +
        (1 - random_cost_prop) * (cost_lose * det_share_factor_j)
      
      ev_j_coal <- reward * p_j_coal / 2 -
        exp_fight_cost_j -
        exp_lose_cost_j * (1 - p_j_coal)
    } else {
      ev_j_coal <- -Inf
    }
    ...
    if (ev_i_coal > ev_i_solo && types[i] == "C") {
      ...
      # reciprocity (if ally exists)
      if (!is.na(i_ally)) coalition_members[[i_ally]] <- i
    } else {
      i_ally <- NA
    }
    ...
    if (ev_j_coal > ev_j_solo && types[j] == "C") {
      ...
      if (!is.na(j_ally)) coalition_members[[j_ally]] <- j
    } else {
      j_ally <- NA
    }
    ...
    if (!is.na(i_ally)) {
      # coalition formed
      # deterministic shares (based on strength proportions)
      total_ia <- stren[i] + stren[i_ally]
      
      det_share_i_fight <- cost_fight * (1 - stren[i] / total_ia)
      det_share_ally_fight <- cost_fight * (1 - stren[i_ally] / total_ia)
      det_share_i_lose <- cost_lose * (1 - stren[i] / total_ia)
      det_share_ally_lose <- cost_lose * (1 - stren[i_ally] / total_ia)
      
      # random allocation (either one pays all or the other pays all)
      cost_ind <- sample.int(2, 1)
      rand_share_i_fight <- cost_fight * (2 - cost_ind)
      rand_share_ally_fight <- cost_fight * (cost_ind - 1)
      rand_share_i_lose <- cost_lose * (2 - cost_ind)
      rand_share_ally_lose <- cost_lose * (cost_ind - 1)
      
      # blended subtraction using random_cost_prop (expected random share is already encoded via sampling)
      i_cost <- ((1 - random_cost_prop) * det_share_i_fight + random_cost_prop * rand_share_i_fight)
      payoff[i] <- payoff[i] - i_cost
      payoff[i_ally] <- payoff[i_ally] - ((1 - random_cost_prop) * det_share_ally_fight + random_cost_prop * rand_share_ally_fight)
      
      # if i loses, subtract loss shares
      if(!i_win) {
        i_cost <- i_cost + ((1 - random_cost_prop) * det_share_i_lose + random_cost_prop * rand_share_i_lose)
        payoff[i] <- payoff[i] - ((1 - random_cost_prop) * det_share_i_lose + random_cost_prop * rand_share_i_lose)
        payoff[i_ally] <- payoff[i_ally] - ((1 - random_cost_prop) * det_share_ally_lose + random_cost_prop * rand_share_ally_lose)
      }
    ...
    if (!is.na(j_ally)) {
      # coalition formed
      total_ja <- stren[j] + stren[j_ally]
      if (total_ja == 0) total_ja <- 1e-12
      det_share_j_fight <- cost_fight * (1 - stren[j] / total_ja)
      det_share_allyj_fight <- cost_fight * (1 - stren[j_ally] / total_ja)
      det_share_j_lose <- cost_lose * (1 - stren[j] / total_ja)
      det_share_allyj_lose <- cost_lose * (1 - stren[j_ally] / total_ja)
      
      cost_ind <- sample.int(2, 1)
      rand_share_j_fight <- cost_fight * (2 - cost_ind)
      rand_share_allyj_fight <- cost_fight * (cost_ind - 1)
      rand_share_j_lose <- cost_lose * (2 - cost_ind)
      rand_share_allyj_lose <- cost_lose * (cost_ind - 1)
      
      j_cost <- ((1 - random_cost_prop) * det_share_j_fight + random_cost_prop * rand_share_j_fight)
      payoff[j] <- payoff[j] - j_cost
      payoff[j_ally] <- payoff[j_ally] - ((1 - random_cost_prop) * det_share_allyj_fight + random_cost_prop * rand_share_allyj_fight)
      
      if(i_win) {
        j_cost <- j_cost + ((1 - random_cost_prop) * det_share_j_lose + random_cost_prop * rand_share_j_lose)
        payoff[j] <- payoff[j] - ((1 - random_cost_prop) * det_share_j_lose + random_cost_prop * rand_share_j_lose)
        payoff[j_ally] <- payoff[j_ally] - ((1 - random_cost_prop) * det_share_allyj_lose + random_cost_prop * rand_share_allyj_lose)
      }
  ...
  return(dat_simu = tibble::tibble(dat_bouts = list(dat_bouts),
                                   strength = list(stren),
                                   payoff = list(payoff)))
}
...
simu_condi <- expand_grid(N = 100,
                          ...
                          random_cost_prop = c(0, 0.2, 0.6, 1),
...
simu_condi <- expand_grid(N = 100,
                          pC = c(0.1, 0.2),
                          round_bouts = c(500, 1000),
                          random_cost_prop = c(0, 0.2, 0.4, 0.6, 0.8, 1),
                          strength_shape = c(1, 2, 3, 4),
                          strength_rate = 1,
                          reward = 1,
                          cost_fight = c(0.02, 0.1, 0.2, 0.3),
                          cost_lose = c(0, 0.10, 0.20))

# parallel computing
library(furrr)
plan(multisession, workers = parallel::detectCores()) # 12 - 2 = 10

res_list <- future_pmap(.l = simu_condi,
                        .f = function(N, pC, round_bouts, random_cost_prop,
                                      strength_shape, strength_rate,
                                      reward, cost_fight, cost_lose) {
                          coalitional_aggres_single_gen(
                            N = N, pC = pC, round_bouts = round_bouts, 
                            random_cost_prop = random_cost_prop,
                            strength_shape = strength_shape, strength_rate = strength_rate,
                            reward = reward, cost_fight = cost_fight,
...
#save(simu_dat, file = "./code/output/simu_dat_partial_rand.RData")
...
lm1 <- lm(payoff_delta ~ round_bouts + strength_shape + cost_fight + cost_lose + random_cost_prop,
          data = payoff_delta)
...
dat_coal <- dat_coal %>%
  group_by(N, pC, round_bouts, strength_shape, strength_rate, 
           random_cost_prop, reward, cost_fight, cost_lose) %>%
  mutate(sim_id = cur_group_id()) %>%
  ungroup()

# random intercept 
model1 <- glmer(coal_form ~ my_stren + rival_type + stren_delta + 
                  cost_fight + cost_lose + random_cost_prop + random_cost_prop * my_stren +
                  (1 | sim_id),
                data = dat_coal,
